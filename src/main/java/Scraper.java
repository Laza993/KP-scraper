...
public class Scraper {
    ...
    public static void main(String[] args) throws Exception {
    	int counter = 403;
    	for(int i = 17; i <= 176; i++) {
    		String pageLink = "https://slobodan.kpizlog.rs/index.php?action=search&data%5Bpage%5D=" + i;
        	
        	final Document documentPage = Jsoup.connect(pageLink).timeout(3000).get();
        	    	
        	Elements links = documentPage.select("div.listingArea.clearfix a[href]");
        	for(Element row : links) {
        		String link = row.attr("href");
        		producToJSON(link, counter);
